...
def _get_key_length_by_backend(backend):
    ...
    if backend == Backend.PROD:
        return 36
    elif backend in [Backend.BETA, Backend.GAMMA]:
        return 36
    elif backend in [Backend.BETA_APIGW]:
        return 40
    elif backend in [Backend.LOCAL]:
        return 55
    elif backend in [Backend.PRODUCT_SERVER_STAGING]:
        return 59
